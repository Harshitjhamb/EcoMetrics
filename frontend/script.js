const BACKEND_BASE = String(
  (window.BACKEND_BASE || localStorage.getItem("BACKEND_BASE") || "http://127.0.0.1:5001")
).replace(/\/$/, "");
const API_BASE = `${BACKEND_BASE}/api`;
const API_URL = `${API_BASE}/combined_data`;
...
async function updateDashboard(pol, met) {
  ...
  if (selected) {
    const cs = document.getElementById("current-station");
    if (cs) cs.textContent = selected;
  }
  renderMiniChart();
}
  ...
  const render = items => {
    ...
    [...list.children].forEach(el => {
      el.onclick = () => {
        ...
        // ✅ save station selection (name)
        localStorage.setItem("eco_loc", name);
        localStorage.setItem("eco_loc_name", name);

        // ✅ update heading instantly (only exists on dashboard)
        const cs = document.getElementById("current-station");
