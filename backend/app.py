...
def _to_jsonable(val):
    """Convert common DB/python types to JSON-safe primitives."""
    if isinstance(val, datetime):
        # stable, frontend-friendly format (instead of Flask's RFC1123 date format)
    ...
    if isinstance(val, timedelta):
        return _timedelta_to_hhmmss(val)
    return val


def json_safe(obj):
    """Recursively convert dict/list values into JSON-safe primitives."""
...
def normalize_row(row, *, in_place: bool):
    """
    Convert row dict values to JSON-safe primitives.
    - in_place=True mutates the dict (useful for lists of rows)
    - in_place=False returns a new dict (useful for single rows)
    """
    if not row:
        return row
    if in_place:
...
def fix_timedelta(row):
    """Backwards-compatible wrapper: returns a JSON-safe copy of the row dict."""
    return normalize_row(row, in_place=False)

def fix_timedelta_row(row):
    """Backwards-compatible wrapper: mutates a row dict to be JSON-safe."""
    return normalize_row(row, in_place=True)
...
def calculate_aqi(pollutants):
    ...
    # AQI is typically reported as an integer; rounding is more accurate than truncation.
    return round(overall_aqi) if overall_aqi is not None else None, iaqis
...
if DB_PASSWORD is None:
    raise RuntimeError("DB_PASSWORD is not set. Add it to backend/.env (DB_PASSWORD=...)")
...
def get_all_stations():
    try:
        ...
        return jsonify(json_safe(rows)), 200
    except Exception as e:
        print("❌ ERROR:", e)
...
def temp_trend():
    ...
    try:
        with conn.cursor() as cur:
            # Use the latest timestamp available in DB as the "now" anchor.
            # This makes the trend work even when the DB contains historical samples.
            cur.execute(
                """
                SELECT MAX(TIMESTAMP(record_date, record_time)) AS max_dt
                FROM meteorological_data
                """
            )
            max_dt_row = cur.fetchone() or {}
            max_dt = max_dt_row.get("max_dt")

            # NOTE: group by full hour timestamp (date+hour) to avoid mixing different days
            # that share the same hour number (e.g., 01:xx from yesterday and today).
            if max_dt is None:
                rows = []
            else:
                cur.execute(
                    """
                    SELECT 
                        DATE_FORMAT(TIMESTAMP(record_date, record_time), '%%Y-%%m-%%d %%H:00:00') AS hour_slot,
                        AVG(temperature_c) AS temp_avg
                    FROM meteorological_data
                    WHERE TIMESTAMP(record_date, record_time) >= (%s - INTERVAL 12 HOUR)
                    GROUP BY hour_slot
        ...
        result = []
        anchor = max_dt or datetime.now()
        for i in range(11, -1, -1):  # oldest → latest
            slot = (anchor - timedelta(hours=i)).strftime("%Y-%m-%d %H:00:00")
            label = (anchor - timedelta(hours=i)).strftime("%H:00")
            result.append(
                {
                    "slot": slot,
                    "record_time": label,
                    "temperature_c": data_map.get(slot),
                }
            )
...
def get_user():
    # Frontend calls this with user_name; keep backwards-compat with user_id too.
    ...
    try:
        with conn.cursor() as cur:
            ...
            row = cur.fetchone()
            return jsonify(json_safe(row if row else {}))
...
def adv_search():
    ...
    try:
        ...
        cleaned_rows = [fix_timedelta_row(r) for r in rows]
